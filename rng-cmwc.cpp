...
void dstCMWCRNG::Initialize(unsigned int _state_size) {
...
dstCMWCRNG::dstCMWCRNG(unsigned int _state_size) {
    Initialize(_state_size);
}

dstCMWCRNG::dstCMWCRNG() {
    Initialize(DST_RANDOM_CMWC_RNG_DEFAULT_STATE_SIZE);
}
...
void dstCMWCRNG::Seed(unsigned int seed) {
    int i;
    Q[0] = seed;
    Q[1] = seed + PHI;
    Q[2] = seed + PHI + PHI;
...
unsigned int dstCMWCRNG::Random32() {
    uint64_t t;
