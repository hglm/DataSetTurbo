...
#ifndef __DST_RANDOM_H__
#define __DST_RANDOM_H__
...
// dstCMWCRNG::dstCMWCRNG(int state_size)
//     Constructor for the default complement-multiply-with-carry RNG
//     using the specified state size.
//
// Non-class member functions:
//
// inline uint32_t dstCalculateLog2(uint32_t n)
// inline uint16_t dstCalculateLog2(uint16_t n)
// inline uint8_t dstCalculateLog2(uint8_t n)
//     Efficiently calculate floor(log2(n)), valid for n >= 1.
// inline uint32_t dstCalculateBitsNeeded(uint32_t n) {
// inline uint16_t dstCalculateBitsNeeded(uint16_t n) {
// inline uint8_t dstCalculateBitsNeeded(uint8_t n) {
//     Calculate number of bits needed to represent an integer range of [0, n - 1],
//     equal to (floor(log2(n - 1)) + 1).
//
// Configuration.
//
...
// function (dstRNG::RandomInt(int n)) which has the range as parameter and
...
// One (DST_RANDOM_EMPIRICAL) produces random integers of the minimum number
...
// The other strategy (DST_RANDOM_INT_REMAINDER) is to use the remainder
...
#define DST_RANDOM_INT_EMPIRICAL
// #define DST_RANDOM_INT_REMAINDER
...
// default), it can be used directly, with either dstRNG::RandomIntEmpirical()
// or dstRNG::RandomIntRemainder(). One or both of these lines must be
// uncommented.
#define DST_RANDOM_INT_INCLUDE_EMPIRICAL_STRATEGY
// #define DST_RANDOM_INT_INCLUDE_REMAINDER_STRATEGY
...
// One (DST_RANDOM_LOG2_LOOKUP_TABLE) uses a 257-byte lookup-table to look up
// log2 and recognize powers of two in 8-bit chunks.

// The other (DST_RANDOM_CALCULATE_LOG2) strategy uses a fairly large number
...
#define DST_RANDOM_CALCULATE_LOG2
// #define DST_RANDOM_LOG2_LOOKUP_TABLE

// The DST_RANDOM_PREFER_64_BIT_STORAGE option makes the class use
...
// DST_RANDOM_FORCE_64_BIT_STORAGE.

#define DST_RANDOM_PREFER_64_BIT_STORAGE
// #define DST_RANDOM_FORCE_64_BIT_STORAGE

// The DST_RANDOM_OPTIMIZE_ALIGNMENT_AND_PACKING_WITH_GCC option, when
...
#define DST_RANDOM_OPTIMIZE_ALIGNMENT_AND_PACKING_WITH_GCC

// When DST_RANDOM_BITS_NEEDED_VALIDATION_CHECK is defined, code is
...
// #define DST_RANDOM_BITS_NEEDED_VALIDATION_CHECK

// End of compile-time configuration definitions.

#ifdef DST_RANDOM_FORCE_64_BIT_STORAGE

#define DST_STORAGE_SIZE 64

#elifdef __GNUC__

#if (defined(DST_RANDOM_PREFER_64_BIT_STORAGE) && __WORDSIZE == 64)
#define DST_STORAGE_SIZE 64
#endif

#endif

#ifndef DST_STORAGE_SIZE
#define DST_STORAGE_SIZE 32
#endif
...
#ifdef DST_RANDOM_BITS_NEEDED_VALIDATION_CHECK
#define VALIDATE_BITS_NEEDED(n, n_bits) dstValidateBitsNeeded(n, n_bits)
...
extern const unsigned char dst_internal_random_table[257] DST_ALIGNED(256);
#endif
...
// number of bits needed to store a certain range of integers.
    ...
    // the range 0 to n - 1.
    DST_API inline unsigned int dstCalculateLog2(unsigned int n) {
    ...
    DST_API inline unsigned int dstCalculateBitsNeeded(unsigned int n) {
    ...
    DST_API inline unsigned int dstCalculateLog2Max256(unsigned int n) {
    ...
    // Calculate number of bits needed for an integer range of n (log2(n - 1) + 1),
    // when n <= 256.
    DST_API inline unsigned int dstCalculateBitsNeededMax256(unsigned int n) {
    ...
    DST_API inline unsigned int dstCalculateLog2Max65536(unsigned int n) {
    ...
    // Calculate number of bits needed for an integer range of n (log2(n - 1) + 1),
    // when n <= 65536.
    DST_API inline unsigned int dstCalculateBitsNeededMax65536(unsigned int n) {
...
class DST_API DST_PACKED dstRNG {
private :
...
private :
    ...
    unsigned int nu_bits) const {
        ...
        return dst_internal_random_table[(n & (mask << shift_start)) >> shift_start];
    }
...
#endif
#ifdef DST_RANDOM_CALCULATE_LOG2
    ...
    // When DST_RANDOM_INT_INCLUDE_REMAINDER_STRATEGY is enabled, the following
    ...
    unsigned int RandomIntRemainderSlowBitsMax256(unsigned int n) {
        ...
        unsigned int shift = dstCalculateBitsNeededMax256(n);
...
public :
...
#endif
#ifdef DST_RANDOM_INT_INCLUDE_EMPIRICAL_STRATEGY
    ...
    inline unsigned int RandomIntEmpirical(unsigned int n) {
        ...
        {
#ifdef DST_RANDOM_CALCULATE_LOG2
            shift = dstCalculateBitsNeeded(n);
#else   // DST_RANDOM_LOG2_LOOKUP_TABLE
    ...
    inline unsigned int RandomIntEmpiricalMax256(unsigned int n) {
        ...
        {
#ifdef DST_RANDOM_CALCULATE_LOG2
            shift = dstCalculateBitsNeededMax256(n);
#else   // DST_RANDOM_LOG2_LOOKUP_TABLE
            shift = GetBitsNeededLookupTable(n);
#endif
            if (((unsigned int)1 << shift) == n) {
    ...
    inline unsigned int RandomIntEmpiricalMax65536(unsigned int n) {
        ...
        {
#ifdef DST_RANDOM_CALCULATE_LOG2
            shift = dstCalculateBitsNeededMax65536(n);
#else   // DST_RANDOM_LOG2_LOOKUP_TABLE
            shift = GetBitsNeededLookupTable(n);
#endif
            if (((unsigned int)1 << shift) == n) {
...
#ifdef DST_RANDOM_INT_EMPIRICAL
    ...
    inline void RandomIntGeneralPrepareForRepeat(unsigned int n) {
        if (n == GetLastGeneralRange())
            return;
        int shift = dstCalculateBitsNeeded(n);
...
#define DST_RANDOM_CMWC_RNG_DEFAULT_STATE_SIZE 8

class DST_API dstCMWCRNG : public dstRNG {
private :
    unsigned int *Q;
    unsigned int c;
    int _index;
...
public :
    dstCMWCRNG();
    dstCMWCRNG(unsigned int state_size);
    ~dstCMWCRNG();
